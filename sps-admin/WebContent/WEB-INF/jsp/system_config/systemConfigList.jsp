...
<div class="cont">
  ...
  <a href="javascript:void(0);" class="this">系统参数设置</a></span> <span class="tab-two"></span></div>
  <form id="queryForm" action="<%=request.getContextPath()%>/admin/systemConfig/List" method="post" >
  <div class="orders">
    <ul>
      <li>
       <span>参数编码</span>
         <span class="order_input size1">
          <input name="paramCode" type="text" id="paramCode" value="${paramCode}" style="width:160px;"/>
        </span>
       <span>参数名称</span>
         <span class="order_input size1">
          <input name="paramName" type="text" id="paramName" value="${paramName}" style="width:160px;"/>
        </span>
        <span class="btn_search">
          <input name="" type="submit"  value="搜索" style="cursor:pointer;"/>
        </span>
        <span class="btn_search">
      	  <a href="<%=request.getContextPath()%>/admin/systemConfig/addSystemConfig">添加系统参数</a>
      	 </span>
       </li>
    </ul>
  </div>
  </form>
  <form action="<%=request.getContextPath()%>/admin/systemConfig/List" method="post" id="ListForm" >
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="content">
      ...
      <tr>
	        ...
	        <td align="center" class="blue xiahua">
	        	<a href="<%=request.getContextPath()%>/admin/systemConfig/show?id=${r.id}">编辑</a>
	        	  &nbsp;&nbsp;|&nbsp;&nbsp;
...
<div class="fenye">
  ...
  <a href="<%=request.getContextPath()%>/admin/systemConfig/List?paramName=${paramName}&paramCode=${paramCode}">首页</a>
  <a href="<%=request.getContextPath()%>/admin/systemConfig/List?page_num=${paging_vo.prePage}&paramName=${paramName}&paramCode=${paramCode}">上一页</a>
  <c:forEach begin="${paging_vo.pageNumStart }" end="${paging_vo.pageNumEnd }" var="pageIndex">
  <c:if test="${paging_vo.currentPage==pageIndex }">
                <a style="background-color:#3b9acb;color:#fff" href="<%=request.getContextPath()%>/admin/systemConfig/List?page_num=${paging_vo.prePage}&paramName=${paramName}&paramCode=${paramCode}">
                    <span>${pageIndex}</span>
               </a>
  </c:if>
  <c:if test="${paging_vo.currentPage!=pageIndex }">        
  <a href="<%=request.getContextPath()%>/admin/systemConfig/List?page_num=${pageIndex}&paramName=${paramName}&paramCode=${paramCode}">
         <span>${pageIndex}</span>
         </a>
  </c:if>
  </c:forEach>
  <a href="<%=request.getContextPath()%>/admin/systemConfig/List?page_num=${paging_vo.nextPage}&paramName=${paramName}&paramCode=${paramCode}">下一页</a>
  <a href="<%=request.getContextPath()%>/admin/systemConfig/List?page_num=${paging_vo.totalPage}&paramName=${paramName}&paramCode=${paramCode}">尾页</a>
...
$(function()
{
	$("#content a[name='del']").click(function()
	{
		...
		}, function()
		{
			$.ajax({
        		type: "post",
        		url: "<%=request.getContextPath()%>/admin/systemConfig/delete?id="+id,
        		cache:false,
